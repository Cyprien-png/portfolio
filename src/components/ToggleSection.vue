...
<template>
  <div
    ...
    <div
      ...
      :class="isOpen ? 'translate-y-0' : 'translate-y-[98%]'">
    </div>
    ...
    <div class="relative z-10 w-full h-full grid grid-cols-3 items-center text-white mix-blend-difference">
      ...
      <div class="h-full flex justify-center items-center">
        <ArrowIcon class="h-[2dvw] block transition-all" :class="isOpen ? 'rotate-90' : 'rotate-0'" />
      </div>
    </div>
  </div>

  <div class="w-full transition-all overflow-hidden bg-black text-white" :class="isOpen ? 'h-full p-6' : 'h-0'">
